...
from core.entities import ClassGroup as CoreClassGroup
...
from database.models import ClassGroup, Lesson, Room, SchoolClass, Teacher


class SqlAlchemyTimetableRepository:
    """Adapter odczytujący bazę SQLite do modelu domenowego."""
    ...
    def load_timetable(self) -> Timetable:
        ...
        class_rows = list(self.session.scalars(select(SchoolClass).order_by(SchoolClass.name)))
        group_rows = list(
            self.session.scalars(
                select(ClassGroup)
                .options(joinedload(ClassGroup.school_class))
                .order_by(ClassGroup.school_class_id, ClassGroup.name)
            )
        )
        ...
        return Timetable(
            lessons=[
                CoreLesson(
                    ...
                    group_name=(row.groups[0].name if row.groups else row.group_name),
            ...
            groups=[CoreClassGroup(row.school_class.name, row.name, row.group_type) for row in group_rows],
