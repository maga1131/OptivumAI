...
lesson_class_groups = Table(
    ...
    Column("class_group_id", ForeignKey("class_groups.id", ondelete="CASCADE"), primary_key=True),
)
...
class SchoolClass(Base):
    ...
    groups: Mapped[list["ClassGroup"]] = relationship(back_populates="school_class", cascade="all, delete-orphan")


class ClassGroup(Base):
    __tablename__ = "class_groups"
    __table_args__ = (UniqueConstraint("school_class_id", "name", name="uq_class_group"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    school_class_id: Mapped[int] = mapped_column(ForeignKey("school_classes.id", ondelete="CASCADE"), index=True)
    name: Mapped[str] = mapped_column(String(60))
    group_type: Mapped[Optional[str]] = mapped_column(String(40), nullable=True)

    school_class: Mapped["SchoolClass"] = relationship(back_populates="groups")
    lessons: Mapped[list["Lesson"]] = relationship(secondary=lesson_class_groups, back_populates="groups")
...
class Lesson(Base):
    ...
    groups: Mapped[list["ClassGroup"]] = relationship(secondary=lesson_class_groups, back_populates="lessons")
